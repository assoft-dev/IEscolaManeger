...
namespace IEscolaDesktop.View.Forms
{
    public partial class frmUsuarios : XtraUserControl
    {
        ...
        List<Usuarios>  UsuariosOriginalList;
        ...
        public frmUsuarios()
        {
            ...
            UsuariosOriginalList = new List<Usuarios>();
        ...
        private void BtnPDF_Click(string Extension)
        {
            ...
            {
                //Busca dos valores
                var buscas = usuariosBindingSource.DataSource as List<Usuarios>;
                if (buscas.Count != 0)
                {
                    rep.GetReport(new rptUsuarios(buscas), "." + Extension, Caminho);
        ...
        private void GridControl1_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                var result = usuariosBindingSource.Current as Usuarios;

                var forms = OpenFormsDialog.ShowForm(null,
                    new frmUsuariosAdd(result ?? null));
        ...
        private void LeituraFilter()
        {
            var data = UsuariosOriginalList.FindAll(x => x.FullName.ToUpper().Contains(txtPesquisar.Text.ToUpper()) ||
                                                         x.Email.ToUpper().Contains(txtPesquisar.Text.ToUpper()));
            usuariosBindingSource.DataSource = data;
        }

        private async void LeituraInicial()
        {
            UsuariosOriginalList = await usuariosRepository.GetAll();
            usuariosBindingSource.DataSource = UsuariosOriginalList;
        ...
        {
            var user = usuariosBindingSource.DataSource as List<Usuarios>;
            if (user != null)
                GlobalReport.GetReport(new rptUsuarios(user), false);
        }
