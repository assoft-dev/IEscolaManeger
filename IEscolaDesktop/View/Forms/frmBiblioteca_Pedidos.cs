...
{
    public partial class frmBiblioteca_Pedidos : XtraUserControl
    {
        IPedidos dataRepository;

        List<Pedidos>  DataOriginalList;

        AlertControl alert = null;

        public frmBiblioteca_Pedidos()
        {
            InitializeComponent();
            dataRepository = new PedidosRepository();
            DataOriginalList = new List<Pedidos>();
        ...
        private void BtnPDF_Click(string Extension)
        {
            ...
            {
                //Busca dos valores
                var buscas = pedidosBindingSource.DataSource as List<Pedidos>;
                if (buscas.Count != 0)
                {
                    rep.GetReport(new rptPedidos(buscas), "." + Extension, Caminho);
        ...
        private void LeituraFilter()
        {
            var data = DataOriginalList.FindAll(x => x.Descricao.ToUpper().Contains(txtPesquisar.Text.ToUpper()) ||
                                                         ...
                                                         x.DocNumero.ToUpper().Contains(txtPesquisar.Text.ToUpper()) ||
                                                         x.PedidosDocuments.ToString().ToUpper().Contains(txtPesquisar.Text.ToUpper()) ||
                                                         x.PedidosEstado.ToString().ToUpper().Contains(txtPesquisar.Text.ToUpper()) );
            pedidosBindingSource.DataSource = data;
        }

        private async void LeituraInicial()
        {
            DataOriginalList = await dataRepository.GetAll();
            pedidosBindingSource.DataSource = DataOriginalList;
        ...
        {
            var user = pedidosBindingSource.DataSource as List<Pedidos>;
            if (user != null)
                GlobalReport.GetReport(new rptPedidos(user), false);
        }
