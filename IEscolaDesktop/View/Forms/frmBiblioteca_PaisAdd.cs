...
{
    public partial class frmBiblioteca_PaisAdd : XtraUserControl
    {
        IPais DataRepository;

        bool IsValidate = false;

        public frmBiblioteca_PaisAdd(Pais usuarios = null)
        {
            InitializeComponent();
            ...
            if (usuarios != null) {
                ...
                txtCodigo.EditValue = usuarios.PaisID;
                txtDescricao.Focus();
            }
            else {
                txtTitulo.Text = "[Novo]";
        ...
        private async void Apagar()
        {
            if (!string.IsNullOrWhiteSpace(txtCodigo.Text))
            {
                ...
                if (msg == DialogResult.Yes)
                {
                    ...
                    var apagar = await DataRepository.Excluir(x => x.PaisID == data);
        ...
        private async Task<bool> ValidationDatabase()
        {
            ...
            if (dataResult != null)
            {
                if (!string.IsNullOrWhiteSpace(txtCodigo.Text))
                {
                    if (dataResult.PaisID != Convert.ToInt32(txtCodigo.Text))
