...
{
    public partial class frmProfessoresCategorias : XtraUserControl
    {
        IProfessoresCategorias DataRepository;

        List<ProfessoresCategorias>  DataOriginalList;

        AlertControl alert = null;

        public frmProfessoresCategorias()
        {
            InitializeComponent();
            DataRepository = new ProfessoresCategoriasRepository();
            DataOriginalList = new List<ProfessoresCategorias>();
        ...
        private void BtnNovo_Click(object sender, EventArgs e)
        {
            var forms = OpenFormsDialog.ShowForm(null,
                   new frmProfessoresAreaFormacaoAdd(null));
        ...
        private void GridControl1_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                var result = professoresCategoriasBindingSource .Current as ProfessoresCategorias;

                var forms = OpenFormsDialog.ShowForm(null,
                    new frmProfessoresCategoriasAdd(result ?? null));
        ...
        private void LeituraFilter()
        {
            ...
            professoresCategoriasBindingSource.DataSource = data;
        }

        private async void LeituraInicial()
        {
            DataOriginalList = await DataRepository.GetAll();
            professoresCategoriasBindingSource.DataSource = DataOriginalList;
        }
