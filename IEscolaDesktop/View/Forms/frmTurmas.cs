...
namespace IEscolaDesktop.View.Forms
{
    ...
    {
        ITurmas usuariosRepository;

        List<Turmas>  UsuariosOriginalList;
        ...
        {
            InitializeComponent();
            usuariosRepository = new TurmasRepository();
            UsuariosOriginalList = new List<Turmas>();
        ...
        private void BtnPDF_Click(string Extension)
        {
            ...
            {
                //Busca dos valores
                var buscas = turmasBindingSource.DataSource as List<Turmas>;
                if (buscas.Count != 0)
                {
        ...
        private void GridControl1_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                var result = turmasBindingSource.Current as Turmas;

                var forms = OpenFormsDialog.ShowForm(null,
                    new frmTurmasAdd(result ?? null));
        ...
        private void LeituraFilter()
        {
            var data = UsuariosOriginalList.FindAll(x => x.Descricao.ToUpper().Contains(txtPesquisar.Text.ToUpper()) ||
                                                         ...
                                                         x.Cursos.Descricao.ToUpper().Contains(txtPesquisar.Text.ToUpper()) ||
                                                         x.Periodos.Descricao.ToUpper().Contains(txtPesquisar.Text.ToUpper()) ||
                                                         x.Salas.Descricao.ToUpper().Contains(txtPesquisar.Text.ToUpper()) );
            turmasBindingSource.DataSource = data;
        }

        private async void LeituraInicial()
        {
            UsuariosOriginalList = await usuariosRepository.GetAll();
            turmasBindingSource.DataSource = UsuariosOriginalList;
        ...
        {
            var user = turmasBindingSource.DataSource as List<Turmas>;
            if (user != null)
