...
{
    public partial class frmBiblioteca_CategoriasAdd : XtraUserControl
    {
        ICategorias DataRepository;
        bool IsValidate = false;

        public frmBiblioteca_CategoriasAdd(Categorias usuarios = null)
        {
            InitializeComponent();

            DataRepository = new CategoriasRepository();
            ...
            if (usuarios != null) {     
                ...
                txtSiglas.EditValue = usuarios.Comentarios;
                txtCodigo.EditValue = usuarios.CategoriasID;
        ...
        private async void Apagar()
        {
            if (!string.IsNullOrWhiteSpace(txtCodigo.Text))
            {
                ...
                if (msg == DialogResult.Yes)
                {
                    ...
                    var apagar = await DataRepository.Excluir(x => x.CategoriasID == data);
        ...
        private async void Guardar()
        {
            ...
            if (!await ValidationDatabase())
            {
                ...
                // save Data
                var data = new Categorias
                {
                    CategoriasID = ID,
                    Descricao = txtDescricao.Text.Trim(),
                    Comentarios = txtSiglas.Text.Trim(),
                };

                IsValidate = ID != 0 ? await DataRepository.Guardar(data, X => X.CategoriasID == ID) > 0 :
        ...
        private async Task<bool> ValidationDatabase()
        {
            ...
            if (dataResult != null)
            {
                if (!string.IsNullOrWhiteSpace(txtCodigo.Text))
                {
                    if (dataResult.CategoriasID != Convert.ToInt32(txtCodigo.Text))
                    {
