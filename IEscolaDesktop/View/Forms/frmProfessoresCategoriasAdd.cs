...
{
    public partial class frmProfessoresCategoriasAdd : XtraUserControl
    {
        IProfessoresCategorias DataRepository;

        bool IsValidate = false;

        public frmProfessoresCategoriasAdd(ProfessoresCategorias usuarios = null)
        {
            InitializeComponent();

            DataRepository = new ProfessoresCategoriasRepository();
            ...
            if (usuarios != null) {
                ...
                txtCodigo.EditValue = usuarios.ProfessoresCategoriasID;
                txtDescricao.Focus();
            }
            else {
                txtTitulo.Text = "[Novo]";
        ...
        private async void Apagar()
        {
            if (!string.IsNullOrWhiteSpace(txtCodigo.Text))
            {
                ...
                if (msg == DialogResult.Yes)
                {
                    ...
                    var apagar = await DataRepository.Excluir(x => x.ProfessoresCategoriasID == data);
        ...
        private async void Guardar()
        {
            if (!await ValidationDatabase())
            {
                ...
                // save Data
                var data = new ProfessoresCategorias
                {
                    ProfessoresCategoriasID = ID,
                    Descricao = txtDescricao.Text.Trim(),
                };

                IsValidate = ID != 0 ? await DataRepository.Guardar(data, X => X.ProfessoresCategoriasID == ID) > 0 :
        ...
        private async Task<bool> ValidationDatabase()
        {
            ...
            if (dataResult != null)
            {
                if (!string.IsNullOrWhiteSpace(txtCodigo.Text))
                {
                    if (dataResult.ProfessoresCategoriasID != Convert.ToInt32(txtCodigo.Text))
                    {
