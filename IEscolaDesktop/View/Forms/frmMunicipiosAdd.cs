...
{
    public partial class frmMunicipiosAdd : XtraUserControl
    {
        IMunicipios DataRepository;
        IProvincias ProvinciasRepository;

        bool IsValidate = false;

        public frmMunicipiosAdd(Municipios usuarios = null)
        {
            InitializeComponent();

            DataRepository = new MunicipiosRepository();
            ...
            if (usuarios != null) {     
                ...
                txtProvinciasCodigo.EditValue = usuarios.ProvinciasID;
            ...
            this.Load += FrmMunicipiosAdd_Load;
        }

        private async void FrmMunicipiosAdd_Load(object sender, EventArgs e)
        {
            provinciasBindingSource.DataSource = await ProvinciasRepository.GetAll();
        }
        ...
        private async void Guardar()
        {
            if (!await ValidationDatabase())
            {
                ...
                // save Data
                var data = new Municipios
                {
                    MunicipiosID = ID,
                    Referencias = txtDescricao.Text.Trim(),
                    ProvinciasID = (int) txtProvinciasCodigo.EditValue,
                    Detalhes = txtDetalhes.Text.Trim(),
                };

                IsValidate = ID != 0 ? await DataRepository.Guardar(data, X => X.MunicipiosID == ID) > 0 :
                ...
                if (control.Name.Equals(txtDescricao.Name))
                {
                    if (!string.IsNullOrWhiteSpace(txtDescricao.Text))
                    {
                        if (!(string.IsNullOrWhiteSpace(txtProvinciasCodigo.Text) || 
                                                       txtProvinciasCodigo.Text == "[Selecione a Provincia por favor]"))
                        { 
                            windowsUIButtonPanel1.Buttons[1].Properties.Enabled = true;
                        }
                        else {
