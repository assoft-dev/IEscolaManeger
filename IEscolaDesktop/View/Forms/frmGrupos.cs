...
namespace IEscolaDesktop.View.Forms
{
    public partial class frmGrupos : XtraUserControl
    {
        ...
        public frmGrupos()
        {
            ...
            DataOriginalList = new List<Grupos>();
        ...
        private void BtnPDF_Click(string Extension)
        {
            ...
            {
                //Busca dos valores
                var buscas = gruposBindingSource.DataSource as List<Grupos>;
                if (buscas.Count != 0)
                {
                    rep.GetReport(new rptGrupos(buscas), "." + Extension, Caminho);
        ...
        private void GridControl1_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                var result = gruposBindingSource.Current as Grupos;

                var forms = OpenFormsDialog.ShowForm(null,
                    new frmGruposAdd(result ?? null));
        ...
        {
            var data = DataOriginalList.FindAll(x => x.Detalhes.ToUpper().Contains(txtPesquisar.Text.ToUpper()) ||
                                                         x.Descricao.ToUpper().Contains(txtPesquisar.Text.ToUpper()));
            gruposBindingSource.DataSource = data;
        }

        private async void LeituraInicial()
        {
            DataOriginalList = await DataRepository.GetAll();
            gruposBindingSource.DataSource = DataOriginalList;
        ...
        {
            var user = gruposBindingSource.DataSource as List<Grupos>;
            if (user != null)
                GlobalReport.GetReport(new rptGrupos(user), false);
        }
