...
{
    public partial class frmBiblioteca_Categorias : XtraUserControl
    {
        ICategorias DataRepository;

        List<Categorias>  DataOriginalList;

        AlertControl alert = null;

        public frmBiblioteca_Categorias()
        {
            InitializeComponent();
            DataRepository = new CategoriasRepository();
            DataOriginalList = new List<Categorias>();
        ...
        private void BtnNovo_Click(object sender, EventArgs e)
        {
            var forms = OpenFormsDialog.ShowForm(null,
                   new frmBiblioteca_CategoriasAdd(null));
        ...
        private void GridControl1_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                var result = categoriasBindingSource.Current as Categorias;

                var forms = OpenFormsDialog.ShowForm(null,
                    new frmBiblioteca_CategoriasAdd(result ?? null));
        ...
        private void LeituraFilter()
        {
            var data = DataOriginalList.FindAll(x => x.Descricao.ToUpper().Contains(txtPesquisar.Text.ToUpper()) ||
                                                         x.Comentarios.ToUpper().Contains(txtPesquisar.Text.ToUpper()));
            categoriasBindingSource.DataSource = data;
        }

        private async void LeituraInicial()
        {
            DataOriginalList = await DataRepository.GetAll();
            categoriasBindingSource.DataSource = DataOriginalList;
