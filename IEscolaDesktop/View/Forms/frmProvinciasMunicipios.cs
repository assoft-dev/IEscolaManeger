...
{
    public partial class frmProvinciasMunicipios : XtraUserControl
    {
        IProvinciasMunicipios DataRepository;

        List<ProvinciasMunicipios>  DataOriginalList;

        AlertControl alert = null;

        public frmProvinciasMunicipios()
        {
            InitializeComponent();
            DataRepository = new ProvinciasMunicipiosRepository();
            DataOriginalList = new List<ProvinciasMunicipios>();
        ...
        private void BtnNovo_Click(object sender, EventArgs e)
        {
            var forms = OpenFormsDialog.ShowForm(null,
                   new frmProvinciasMunicipiosAdd(null));
        ...
        private void GridControl1_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                var result = provinciasMunicipiosBindingSource.Current as ProvinciasMunicipios;

                var forms = OpenFormsDialog.ShowForm(null,
                    new frmProvinciasMunicipiosAdd(result ?? null));
        ...
        {
            var data = DataOriginalList.FindAll(x => x.Provincias.Referencias.ToUpper().Contains(txtPesquisar.Text.ToUpper()) ||
                                                         x.Municios.Referencias.ToUpper().Contains(txtPesquisar.Text.ToUpper()));
            provinciasMunicipiosBindingSource.DataSource = data;
        }

        private async void LeituraInicial()
        {
            DataOriginalList = await DataRepository.GetAll();
            provinciasMunicipiosBindingSource.DataSource = DataOriginalList;
