...
{
    public partial class frmSalasAdd : XtraUserControl
    {
        ISalas DataRepository;

        bool IsValidate = false;

        public frmSalasAdd(Salas usuarios = null)
        {
            InitializeComponent();
            ...
            if (usuarios != null) {
                ...
                txtCodigo.EditValue = usuarios.SalasID;
                txtDescricao.Focus();
            }
            else {
                txtTitulo.Text = "[Novo]";
        ...
        private async void Apagar()
        {
            if (!string.IsNullOrWhiteSpace(txtCodigo.Text))
            {
                ...
                if (msg == DialogResult.Yes)
                {
                    ...
                    var apagar = await DataRepository.Excluir(x => x.SalasID == data);
        ...
        private async void Guardar()
        {
            if (!await ValidationDatabase())
            {
                ...
                IsValidate = ID != 0 ? await DataRepository.Guardar(data, X => X.SalasID == ID) > 0 :
